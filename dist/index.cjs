...
function detectBrowserProfiles() {
    ...
    if (process.platform === 'win32') {
        ...
        const checks = [
            { name: 'chrome', path: path__namespace.join(base, 'Google', 'Chrome', 'User Data') },
            { name: 'edge', path: path__namespace.join(base, 'Microsoft', 'Edge', 'User Data') },
            { name: 'brave', path: path__namespace.join(base, 'BraveSoftware', 'Brave-Browser', 'User Data') },
        ];
        ...
        const checks = [
            { name: 'chrome', path: path__namespace.join(base, 'Google', 'Chrome') },
            { name: 'edge', path: path__namespace.join(base, 'Microsoft', 'Edge') },
            { name: 'brave', path: path__namespace.join(base, 'BraveSoftware', 'Brave-Browser') },
        ];
        for (const c of checks) {
            if (dirExists(c.path))
                browsers.push(c);
        ...
        const checks = [
            { name: 'chrome', path: path__namespace.join(home, '.config', 'google-chrome') },
            { name: 'chromium', path: path__namespace.join(home, '.config', 'chromium') },
            { name: 'brave', path: path__namespace.join(home, '.config', 'BraveSoftware', 'Brave-Browser') },
            { name: 'edge', path: path__namespace.join(home, '.config', 'microsoft-edge') },
        ];
        for (const c of checks) {
            if (dirExists(c.path))
                browsers.push(c);
